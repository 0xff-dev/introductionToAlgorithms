...
func distance593(a, b []int) int {
	x := a[0] - b[0]
	y := a[1] - b[1]
	return x*x + y*y
}
func edgesEqual(x1, y1, x2, y2 []int) bool {
	x1x2 := distance593(x1, x2)
	x1y2 := distance593(x1, y2)

	if x1x2 != x1y2 {
		return false
	}
	y1x2 := distance593(y1, x2)
	y1y2 := distance593(y1, y2)
	if y1x2 != y1y2 {
		return false
	}
	if y1x2 != x1y2 {
	...
	return distance593(x1, y1) == distance593(x2, y2)
}
