...
func countBalancedPermutations(num string) int {
	...
	for i := range comb {
		...
		for j := 1; j < i; j++ {
			comb[i][j] = (comb[i-1][j] + comb[i-1][j-1]) % MOD3343
		}
	}
		...
		for oddCnt := min(psum, maxOdd); oddCnt >= max(0, psum-(n-maxOdd)); oddCnt-- {
			...
			for curr := min(totSum, target); curr >= max(0, totSum-target); curr-- {
				...
				for j := max(0, cnt[i]-evenCnt); j <= min(cnt[i], oddCnt) && i*j <= curr; j++ {
					...
					ways := comb[oddCnt][j] * comb[evenCnt][cnt[i]-j] % MOD3343
					res = (res + ways*f[curr-i*j][oddCnt-j]%MOD3343) % MOD
				}
				f[curr][oddCnt] = res % MOD3343
			}
		}
	}
