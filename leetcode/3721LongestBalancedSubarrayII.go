...
type SegmentTree3721Node3721 struct {
	minValue int
	maxValue int
	lazyTag  *LazyTag
}

func NewSegmentTree3721Node3721() *SegmentTree3721Node3721 {
	return &SegmentTree3721Node3721{
		minValue: 0,
		maxValue: 0,
		lazyTag:  &LazyTag{},
	}
...
type SegmentTree3721 struct {
	n    int
	tree []*SegmentTree3721Node3721
}

func NewSegmentTree3721(data []int) *SegmentTree3721 {
	n := len(data)
	tree := make([]*SegmentTree3721Node3721, n*4+1)
	for i := range tree {
		tree[i] = NewSegmentTree3721Node3721()
	}
	seg := &SegmentTree3721{n: n, tree: tree}
	seg.build(data, 1, n, 1)
	return seg
}

func (seg *SegmentTree3721) Add(l, r, val int) {
	tag := &LazyTag{toAdd: val}
	seg.update(l, r, tag, 1, seg.n, 1)
}

func (seg *SegmentTree3721) FindLast(start, val int) int {
	if start > seg.n {
		return -1
	}
...
func (seg *SegmentTree3721) applyTag(i int, tag *LazyTag) {
	seg.tree[i].minValue += tag.toAdd
	seg.tree[i].maxValue += tag.toAdd
	seg.tree[i].lazyTag.Add(tag)
}

func (seg *SegmentTree3721) pushdown(i int) {
...
func (seg *SegmentTree3721) pushup(i int) {
	left := seg.tree[i<<1]
...
func (seg *SegmentTree3721) build(data []int, l, r, i int) {
	if l == r {
...
func (seg *SegmentTree3721) update(targetL, targetR int, tag *LazyTag, l, r, i int) {
...
func (seg *SegmentTree3721) find(targetL, targetR, val, l, r, i int) int {
...
func longestBalanced3721(nums []int) int {
	...
	seg := NewSegmentTree3721(prefixSum)
